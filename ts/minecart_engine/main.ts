...
system.afterEvents.scriptEventReceive.subscribe(async (ev)=>{
    ...
    switch(ev.id){
      case "tcmb:event":
        ...
        switch(evdata.name){
            case "rideSignal":
              if(typeof evdata.entity != "object") return;
              train_unknown = world.getEntity(evdata.entity.id);
              if(train_unknown instanceof Entity) train = train_unknown ;else return;
              if(typeof train != "undefined" && train.typeId == "tcmb:tcmb_car" && evdata.player){
